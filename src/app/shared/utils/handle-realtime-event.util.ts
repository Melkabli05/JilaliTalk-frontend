...
/** Handles the user-action realtime events that require ConfirmService dialogs.
 *  Pure function — no Angular DI, no signals. Callable from any context.
 *
 *  Why not a service?  Each page owns the room lifecycle and the API calls.
 *  Extracting the switch eliminates duplication without introducing a new DI boundary. */
